...
function play(client, guild, song, info, m, message) {
    ...
    const imbed = new Discord.MessageEmbed()
        ...
        .addField('노래 제목', info.title)
        .addField('노래 URL', info.video_url)
        .addField('상세 설명', info.description)
        .addField('재생 시간', `${Math.floor(info.length_seconds / 3600)}시간 ${Math.floor(parseInt(Math.floor((Math.floor(info.length_seconds - Math.floor(info.length_seconds / 3600) * 3600) / 60)).toString().split('.')[0]))}분 ${info.length_seconds - Math.floor(info.length_seconds / 60) * 60}초`)
        .addField('제작자 id', info.author.id)
        .addField('제작자 채널 이름', info.author.name)
...
module.exports = {
    ...
    run: async function (client, message, args, option) {
        ...
        const embed = new Discord.MessageEmbed()
            ...
            .setColor(0xffff00)
            .addField('검색어 또는 노래 URL', args.slice(1).join(' '))
            .setFooter(message.author.tag, message.author.avatarURL({
                dynamic: true
            }))
            .setTimestamp()
...
에러 내용: ${err}`);
                    ...
                    const emved = new Discord.MessageEmbed()
                        ...
                        .addField('노래 제목', songInfo.title)
                        .addField('노래 URL', songInfo.video_url)
                        .addField('상세 설명', songInfo.description)
                        .addField('재생 시간', `${Math.floor(songInfo.length_seconds / 3600)}시간 ${Math.floor(parseInt(Math.floor((Math.floor(songInfo.length_seconds - Math.floor(songInfo.length_seconds / 3600) * 3600) / 60)).toString().split('.')[0]))}분 ${songInfo.length_seconds - Math.floor(songInfo.length_seconds / 60) * 60}초`)
                        .addField('제작자 id', songInfo.author.id)
                        .addField('제작자 채널 이름', songInfo.author.name)
