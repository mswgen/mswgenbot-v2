...
module.exports = {
    ...
    run: async function (client, message, args, option) {
        await axios.post('https://is.gd/hihhls').then(async function (response) {
            const getJSON = response.data;
            await axios.get('https://search.naver.com/search.naver?sm=top_hty&fbm=1&ie=utf8&query=%EC%BD%94%EB%A1%9C%EB%82%9819').then(async function (res) {
                if (res.status === 200) {
                    const $ = cheerio.load(res.data)
                    let c = new Array()

                    $('.graph_view > .box').each(async function (i, d) {
