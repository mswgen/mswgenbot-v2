...
module.exports = {
    ...
    run: async function (client, message, args, option) {
        const embed = new Discord.MessageEmbed()
            ...
            .addField('디스코드 가입일', parseDate(message.author.createdAt))
            .addField('서버 참가일', parseDate(message.member.joinedAt))
            .addField('봇 여부', message.author.bot)
            .addField('디스코드 클라이언트 상태', area(message.author))
            .addField('현재 상태 메세지, 게임 목록', stat(message.author))
            .addField('서버 내 역할', myRoles(message.member.roles.cache, message.guild))
